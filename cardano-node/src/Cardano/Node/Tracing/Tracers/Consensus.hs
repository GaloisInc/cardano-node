...
instance MetaTrace a => MetaTrace [TraceLabelPeer peer a] where
  namespaceFor (a : tl') = (nsCast . namespaceFor) (a : tl')
...
instance MetaTrace  (ForgeTracerType blk) where
  ...
  namespaceFor (Right ev) =
    nsCast (namespaceFor ev)

  severityFor ns (Just (Right ev')) =
    severityFor (nsCast ns) (Just ev')
  severityFor ns@(Namespace _ ["StartLeadershipCheckPlus"]) Nothing =
    severityFor (nsCast ns :: Namespace TraceStartLeadershipCheckPlus) Nothing
  severityFor ns (Just (Left ev')) =
    severityFor (nsCast ns) (Just ev')
  severityFor ns Nothing =
    severityFor (nsCast ns :: Namespace (TraceForgeEvent blk)) Nothing

  detailsFor ns (Just (Right ev')) =
    detailsFor (nsCast ns) (Just ev')
  detailsFor ns@(Namespace _ ["StartLeadershipCheckPlus"]) Nothing =
    detailsFor (nsCast ns :: Namespace TraceStartLeadershipCheckPlus) Nothing
  detailsFor ns (Just (Left ev')) =
    detailsFor (nsCast ns) (Just ev')
  detailsFor ns Nothing =
    detailsFor (nsCast ns :: Namespace (TraceForgeEvent blk)) Nothing

  privacyFor ns (Just (Right ev')) =
    privacyFor (nsCast ns) (Just ev')
  privacyFor ns@(Namespace _ ["StartLeadershipCheckPlus"]) Nothing =
    privacyFor (nsCast ns :: Namespace TraceStartLeadershipCheckPlus) Nothing
  privacyFor ns (Just (Left ev')) =
    privacyFor (nsCast ns) (Just ev')
  privacyFor ns Nothing =
    privacyFor (nsCast ns :: Namespace (TraceForgeEvent blk)) Nothing

  metricsDocFor ns@(Namespace _ ["StartLeadershipCheckPlus"]) =
    metricsDocFor(nsCast ns :: Namespace TraceStartLeadershipCheckPlus)
  metricsDocFor ns =
    metricsDocFor (nsCast ns :: Namespace (TraceForgeEvent blk))

  documentFor ns@(Namespace _ ["StartLeadershipCheckPlus"]) =
    documentFor(nsCast ns :: Namespace TraceStartLeadershipCheckPlus)
  documentFor ns =
    documentFor (nsCast ns :: Namespace (TraceForgeEvent blk))

  allNamespaces = map nsCast
    (allNamespaces :: [Namespace TraceStartLeadershipCheckPlus])
    <> map nsCast
...
instance MetaTrace TraceStartLeadershipCheckPlus where
  namespaceFor TraceStartLeadershipCheckPlus {} =
    Namespace [] ["StartLeadershipCheckPlus"]

  severityFor (Namespace _ ["StartLeadershipCheckPlus"]) _ = Just Info
  severityFor _ _ = Nothing
  ...
  allNamespaces = [Namespace [] ["StartLeadershipCheckPlus"]]
